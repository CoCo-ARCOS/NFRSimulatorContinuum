...
struct config *read_config(const char *file_name)
{
    ...
    configuration->aes_key_bits = 256; // Default AES/ChaCha20 key size.
...
{
    (void)workers;
    printf("Interpolation-only mode: skipping queue-estimator agent startup for %s agents.\n",
...
static int run_queue_estimator(const char *container_prefix, int worker_id, double mean_interarrival, double mean_service, int samples, const char *result_file, double *total_time)
{
    if (samples <= 0 || mean_service <= 0.0)
        return -1;
    if (total_time)
        *total_time = mean_service * (double)samples;

    printf("Interpolation-only mode: bypassing queue estimator for worker %d - %s; mean interarrival %f, mean service %f, samples %d, raw total %f\n",
           worker_id,
           container_prefix ? container_prefix : "n/a",
           mean_interarrival,
           mean_service,
           samples,
           total_time ? *total_time : mean_service * (double)samples);
    ...
    if (result_file && result_file[0] != '\0')
    {
        ...
        if (path)
        {
            ...
            snprintf(result_path, sizeof(result_path), "%s/results/%s", path, result_file);
            FILE *out = fopen(result_path, "a");
            if (out)
            {
                fprintf(out, "queue_bypassed\tworker=%d\tmean_interarrival=%f\tmean_service=%f\tsamples=%d\ttotal=%f\n",
                        worker_id,
                        mean_interarrival,
                        mean_service,
                        samples,
...
static void hash_task_time(struct worker *my_data, int result_stage)
{
    ...
    if (my_data->sizeWorker > 0)
    {
        ...
        for (j = 0; j < my_data->sizeWorker; ++j)
        {
            ...
            printf("t_read %f t_write %f size %ld\n", my_data->b_fs_write, my_data->b_fs_write, size_before);

            if (my_data->b_fs_read > 0.0)
                t_read = (double)size_before / (my_data->b_fs_read * 16);

            float hash_time = hashingStageAlgo((double)size_before, my_data->task_algorithm);
            if (hash_time <= 0.0f)
                hash_time = hashingStage((double)size_before);
            if (my_data->b_fs_write > 0.0)
                t_write = (double)size_before / (my_data->b_fs_write * 16);
            
            printf("read time %lf, write time %lf hash time %lf suma %lf\n", t_read, t_write, hash_time, hash_time + t_read + t_write);
