...
def make_compress_out_app(executors):
    @python_app(executors=executors)
    def compress_out(paths_tuple, output_compressed_path):
        import time
        _t0 = time.time()
...
def make_encrypt_out_app(executors, algo='chacha20'):
    @python_app(executors=executors)
    def encrypt_out(input_compressed, output_encrypted, key_path):
        import time
        _t0 = time.time()
        import os
        from cryptography.hazmat.primitives.ciphers import Cipher, algorithms, modes
        ...
        with open(input_compressed, 'rb') as f_in:
            data = f_in.read()
        ...
        with open(output_encrypted, 'wb') as f_out:
...
def make_encode_out_app(executors):
    @python_app(executors=executors)
    def encode_out(input_encrypted, output_prefix):
        import time
        _t0 = time.time()
        import zfec
        ...
        with open(input_encrypted, 'rb') as f_in:
            data = f_in.read()
        ...
        for i, block in enumerate(encoded_blocks):
            out_path = f"{output_prefix}.block{i}"
...
def make_decode_in_app(executors):
    @python_app(executors=executors)
    def decode_in(input_blocks, output_encrypted):
        import time
        _t0 = time.time()
        import zfec
        ...
        blocknums = []
        pad_len = 0
        for i, bpath in enumerate(input_blocks):
            if i >= k: break # we only need k blocks
            with open(bpath, 'rb') as f:
                if bpath.endswith("block0"):
                    pad_len = int.from_bytes(f.read(4), 'big')
                data = f.read()
                blocks.append(data)
                blocknums.append(int(bpath.split("block")[-1]))
...
def make_decrypt_in_app(executors):
    @python_app(executors=executors)
    def decrypt_in(input_encrypted, output_compressed, key_path):
        import time
        _t0 = time.time()
        from cryptography.hazmat.primitives.ciphers import Cipher, algorithms, modes
...
def make_decompress_in_app(executors):
    @python_app(executors=executors)
    def decompress_in(input_compressed, output_dir):
        import time
        _t0 = time.time()
        import lz4.frame
        import tarfile
        ...
        tar_path = input_compressed + ".tar"
        with open(input_compressed, 'rb') as f_in, open(tar_path, 'wb') as f_out:
        ...
        hash_file = [f for f in extracted_files if f.endswith('.json')][0]
        data_file_or_dir = [f for f in extracted_files if f != hash_file][0]
        
        import time; _t1 = time.time()
        with open('workflow_timing.log', 'a') as _f: _f.write(f'decompress_in,{_t1-_t0:.4f}\n')
        return str(Path(output_dir) / data_file_or_dir), str(Path(output_dir) / hash_file)
