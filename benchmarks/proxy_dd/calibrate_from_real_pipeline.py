...
def calibrate_config(config, application_means, source_interarrival, notes):
    ...
    stage_queue_interarrivals = notes.get("derived_stage_interarrival_seconds", {})
    measured_stage1_interarrival = stage_queue_interarrivals.get("stage1", 0.0)
    ...
    for stage in calibrated.get("stages", []):
        ...
        if stage_name == "stage1":
            if measured_stage1_interarrival <= 0.0:
                stage["arrival_model"] = "burst"
                stage["inter_arrival"] = 0.0
            else:
                stage["inter_arrival"] = round(source_interarrival, 6)
                stage.pop("arrival_model", None)
        elif stage_name in stage_queue_interarrivals and stage_queue_interarrivals[stage_name] > 0.0:
            stage["inter_arrival"] = round(stage_queue_interarrivals[stage_name], 6)
            stage.pop("arrival_model", None)
...
def main():
    ...
    stage_interarrivals = {
        row["stage"]: float_value(row, "mean_interarrival_seconds")
        for row in queue_summary_rows
        if row.get("stage")
    }
    ...
    notes = {
        ...
        "derived_stage_interarrival_seconds": stage_interarrivals,
        "derived_source_interarrival_seconds": source_interarrival,
        "stage1_arrival_model": "burst" if stage_interarrivals.get("stage1", 0.0) <= 0.0 else "rate",
    }
