...
def predict_with_helper(helper_path, workspace_dir, sample):
    cmd = [
        str(helper_path),
        sample["operation"],
...
def validate_samples(samples, helper_path, real_values_dir, mode):
    results = []

    for sample in samples:
        workspace_dir = clone_real_values(real_values_dir, excluded_sample=sample if mode == "leave-one-out" else None)
        try:
            predicted = predict_with_helper(helper_path, workspace_dir, sample)
        finally:
...
def main():
    ...
    results = validate_samples(samples, helper_path, real_values_dir, args.mode)
    summaries, operation_summaries, overall = summarize(results)
    print_summary_table(summaries, operation_summaries, overall, args.mode)
