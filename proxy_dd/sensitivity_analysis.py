        ...
        for item in group:
            ...
            for repeat in range(1, args.repeats + 1):
                ...
                if factor == "payload_size":
                    modified_rows = list(trace_rows)
                    modified_rows[0] = dict(modified_rows[0])
                    modified_rows[0]["SIZE"] = float(item["payload_size"])
                    write_traces_cfg(modified_rows, run_dir / traces_file_name)
                    ...
                    shutil.copy2(baseline_traces_path, run_dir / traces_file_name)
